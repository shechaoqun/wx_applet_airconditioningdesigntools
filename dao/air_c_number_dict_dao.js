...
function getRoomType() {
  ...
  for (var i = 0; i < air_c_number_dict.length; i++) {
    temp_array.push(air_c_number_dict[i].room_type);
  }
  var json = {};
...
function getMachinType(roomType) {
  ...
  for (var i = 0; i < air_c_number_dict.length; i++) {
    var temp_roomType = air_c_number_dict[i].room_type;
    if (roomType == temp_roomType) {
      temp_array.push(air_c_number_dict[i].machin_type);
    }
  }
  var json = {};
  for (var i = 0; i < temp_array.length; i++) {
...
function getHorsesNumber(roomType, machinType) {
  ...
  for (var i = 0; i < air_c_number_dict.length; i++) {
    var temp_roomType = air_c_number_dict[i].room_type;
    var temp_machinType = air_c_number_dict[i].machin_type;
    if (roomType == temp_roomType && machinType == temp_machinType) {
      temp_array.push(air_c_number_dict[i].horses_number);
    }
  }
  var json = {};
  for (var i = 0; i < temp_array.length; i++) {
...
function getData(data) {
  ...
  for (var i = 0; i < air_c_number_dict.length; i++) {
    var room_type = air_c_number_dict[i].room_type;
    var machin_type = air_c_number_dict[i].machin_type;
    var horses_number = air_c_number_dict[i].horses_number;
    if (data.room_type == room_type && data.machin_type == machin_type && data.horses_number == horses_number) {
      data.quantity_cold = air_c_number_dict[i].quantity_cold;
      data.default_num = air_c_number_dict[i].default_num;
      data.price = air_c_number_dict[i].price;
      return data;
    }
  }
